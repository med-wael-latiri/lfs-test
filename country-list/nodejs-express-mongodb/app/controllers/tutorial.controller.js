...
//   if (name === "country") {
//     res.status(500).send(console.error("name exists"));
//   } else {
//     res.status(200).send(console.error("data saved"));
//     t.save();
//   }
...
var countrylist = [
  {
    name: "country",
    UniqueKeys: ["alpha-2", "alpha-3", "country-code", "iso_3166-2"],
    DefaultKey: "alpha-3",
    data: [
      {
        name: "Afghanistan",
...
exports.findAll = async (req, res) => {
  ...
  if (querys && querys.name) {
    const t = await Tutorial.find({ name: querys.name });
    res.send(t);
  } else {
...
exports.create = async (req, res) => {
  const { name } = req.body;
  const t = await Tutorial.findOne({ name: name });

  if (t) {
    res.status(500).send("name already exists");
  } else {
    const aa = new Tutorial(req.body);
